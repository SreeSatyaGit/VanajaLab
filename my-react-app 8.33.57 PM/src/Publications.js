    ...
    <ul style={styles.list}>
        {publications.map((publication, index) => (
            <li key={index} style={styles.listItem}>
                <strong>{publication.authors}</strong> ({publication.date}). {publication.title}. <em>{publication.journal}</em> {publication.volume ? <span>{publication.volume}</span> : ''} {publication.pages ? <span>{publication.pages}</span> : ''}. DOI: {publication.doi ? <a href={`https://doi.org/${publication.doi}`}>{publication.doi}</a> : ''} PMID: {publication.pmid ? <span>{publication.pmid}</span> : ''} PMCID: {publication.pmcid ? <span>{publication.pmcid}</span> : ''}.
            </li>
        ))}
    </ul>
</div>
